...
for value in list_Input:
    ...
    # Get the first digit if a match is found
    first_digit = match.group() if match else None

    # Use regex to find the last digit
    match = re.search(r'\d', value[::-1])

    # Get the last digit if a match is found
    last_digit = match.group() if match else None

    #combine to one number
