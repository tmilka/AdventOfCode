...
def extract_first_last_numbers(s):

    # Create a regular expression pattern using the group of words
    pattern = "(?:\d|one|two|three|four|five|six|seven|eight|nine)"

    # Find all occurrences of numbers or words representing numbers
    matches = re.findall(pattern, s, re.IGNORECASE)

    # Extract the first and last elements from the matches
    first_number = matches[0] if matches else None
    last_number = matches[-1] if matches else None

    #print("Test2")

    if(wordToNumber(first_number) == None):
        pass
    else:
        first_number = wordToNumber(first_number)
    
    if(wordToNumber(last_number) == None):
        pass
    else:
        last_number = wordToNumber(last_number)
...
a1b2c3d4e5f
treb7uchetonetwo"""
#split string into a list based on whitespace

list_Input = input2.split()

#list containig the two digits
results_list = []

#check for first and last number in the list
for value in list_Input:

    first, last = extract_first_last_numbers(value)

    result = str(first) + str(last)
    print(result)
    results_list.append(result)

answer_number = sum(int(num) for num in results_list)
